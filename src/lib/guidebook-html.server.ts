...
// 未满足约束提示块：总览页与受影响日期共用，违规为空时不渲染。
function renderViolationNotice(violations: PlanViolation[], heading: string): string {
  if (violations.length === 0) return "";
  const items = violations.map((violation) => `<li>${escapeHtml(violation.message)}</li>`).join("");
  return `<div class="violation-notice"><div class="violation-notice-heading"><i class="ti ti-alert-triangle"></i><span>${escapeHtml(heading)}</span></div><ul>${items}</ul><p class="violation-notice-foot">已按设定条件尽量降级调整，上方为最终仍未满足的项，出发前请再次确认。</p></div>`;
}
...
function renderOverview(plan: TripPlan): string {
  ...
  const weatherItems = plan.days
    .map((day, index) =>
      day.weather?.trim()
        ? `<li><span>第 ${index + 1} 天 · ${escapeHtml(formatDate(day.date))}</span><strong>${escapeHtml(day.weather.trim())}</strong></li>`
        : "",
    )
    .join("");
  const weatherBlock = weatherItems
    ? `<section class="overview-block"><h3>逐日天气</h3><ul class="weather-list">${weatherItems}</ul></section>`
    : "";
  const violationNotice = renderViolationNotice(
    plan.violations ?? [],
    `本次有 ${(plan.violations ?? []).length} 项未满足你设定的条件`,
  );
  const body = `${violationNotice}${renderSectionHeading("JOURNEY OVERVIEW / 出发前", "旅程概览")}<div class="overview-grid"><div class="overview-item"><i class="ti ti-calendar"></i><span>总天数</span><strong>${plan.meta.days} 天</strong></div><div class="overview-item"><i class="ti ti-route"></i><span>总里程</span><strong>${plan.route.distanceKm > 0 ? `${formatNumber(plan.route.distanceKm)} km` : "待估算"}</strong></div><div class="overview-item"><i class="ti ti-clock"></i><span>交通用时</span><strong>${plan.route.durationMinutes > 0 ? formatDuration(plan.route.durationMinutes) : "待估算"}</strong></div><div class="overview-item"><i class="ti ti-coin"></i><span>预算范围</span><strong>${currency(plan.budget.totalMin)} - ${currency(plan.budget.totalMax)}</strong></div></div><section class="overview-block"><h3>路线节奏</h3><p>${escapeHtml(plan.meta.origin)} 出发，前往 ${escapeHtml(plan.meta.destination)}。${escapeHtml(paceLabel(plan))}，${escapeHtml(transportPreferenceLabel(plan))}，同行人数为 ${escapeHtml(travelerText(plan))}。</p></section>${weatherBlock}<section class="overview-block"><h3>核心亮点</h3><ul class="highlight-list">${highlightItems}</ul></section><section class="overview-block"><h3>兴趣与主题</h3><div class="chip-row">${interestItems}</div></section>${renderDivider()}`;
...
function renderDaySummary(plan: TripPlan, day: TripDay, index: number): string {
  ...
  const dayViolations = (plan.violations ?? []).filter((violation) => violation.day === index + 1);
  const dayViolationNotice = renderViolationNotice(
    dayViolations,
    `本次有 ${dayViolations.length} 项未满足条件`,
  );
  const body = `${renderSectionHeading(`DAY ${String(index + 1).padStart(2, "0")} / 执行与总结`, "执行时间轴")}<div class="day-heading compact"><div><h1>${escapeHtml(day.theme)}</h1></div><div class="day-heading-meta"><span><i class="ti ti-calendar"></i> ${escapeHtml(formatDate(day.date))}</span></div></div><section class="timeline-section">${renderTimeline(day)}</section><section class="day-summary"><h3>今日总结</h3><div class="summary-purpose"><span>今日目的</span><p>${escapeHtml(day.purpose)}</p></div><div class="summary-focus"><h4>核心重点</h4><ol>${focusMarkup}</ol></div>${dayViolationNotice}<div class="summary-cautions"><h4>注意事项</h4><ul>${cautions
    .slice(0, 5)
...
export type GuidebookPageSpec = {
  id: string;
  label: string;
  /** 渲染单页 HTML；只有需要图片的页才会拿到已内联图片的 plan。 */
  render: (plan: TripPlan) => string;
};

/** 路书的页面清单：顺序即 PDF 页序，逐页预览也按它推进。 */
export function guidebookPageSpecs(plan: TripPlan): GuidebookPageSpec[] {
  return [
    { id: "cover", label: "封面", render: (target) => renderCover(target) },
    { id: "overview", label: "旅程概览", render: (target) => renderOverview(target) },
    { id: "route", label: "路线与交通", render: (target) => renderRoute(target) },
    { id: "budget", label: "预算分配", render: (target) => renderBudget(target) },
    ...plan.days.flatMap((_day, index) => [
      {
        id: `day-${index + 1}-map`,
        label: `第 ${index + 1} 天 · 导航与准备`,
        render: (target: TripPlan) => renderDayMapPage(target, target.days[index], index),
      },
      {
        id: `day-${index + 1}-summary`,
        label: `第 ${index + 1} 天 · 执行与总结`,
        render: (target: TripPlan) => renderDaySummary(target, target.days[index], index),
      },
    ]),
    { id: "sources", label: "数据来源与估算说明", render: (target) => renderSources(target) },
    { id: "closing", label: "旅行回望", render: (target) => renderClosing(target) },
    { id: "xuxiake", label: "致当代徐霞客", render: (target) => renderXuxiakePage(target) },
  ];
}

/** 预览专用的字体兜底：不请求 Google Fonts 与 Tabler，改用系统中文字体。 */
const PREVIEW_FALLBACK_CSS =
  ":root{--font-serif:'Songti SC','SimSun','Source Han Serif SC',serif;--font-sans:'PingFang SC','Microsoft YaHei','Source Han Sans SC',sans-serif;--font-mono:'JetBrains Mono','Consolas',monospace}.ti{display:none}";

const GUIDEBOOK_FONT_LINKS =
 ...
 * 文档头：字体、图标与打印样式。
 *
 * `forPreview` 供浏览器内的逐页预览使用——预览不请求 Google Fonts / Tabler CDN，
...
export function renderGuidebookPages(
  ...
  return guidebookPageSpecs(plan).map((spec) => ({
    id: spec.id,
    label: spec.label,
    html: spec.render(plan),
  }));
}

export function renderGuidebookDocument(plan: TripPlan, pagesHtml: string): string {
  return `<!doctype html><html lang="zh-CN"><head>${renderGuidebookHead(plan)}</head><body>${pagesHtml}</body></html>`;
}

export function renderGuidebookHtml(plan: TripPlan): string {
  return renderGuidebookDocument(
    plan,
    renderGuidebookPages(plan)
      .map((page) => page.html)
      .join(""),
  );
}
